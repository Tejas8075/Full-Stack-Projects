...
const ChatContainer = () => {
  ...
  const handleSendImage = async (e) => {
    ...
    reader.onloadend = async () => {
      ...
      // clear the image url in the console after the url to the api
      e.target.value = "";
    }
  ...
  // whenever the selected user changes it will call the getMessages function
  useEffect(() => {
    if (selectedUser) {
  ...
  // It will scroll the web page whenever new message comes
  useEffect(() => {
  ...
  return selectedUser ? (
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            ...
            <div className='text-center text-xs'>
              <img src={msg.senderId === authUser._id ? authUser.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="profile" className='w-7
               rounded-full' />
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      <img src={assets.logo_icon} alt="app_logo" className='max-w-16' />
